...
function neutralizeFormula<T extends string | number | Date | null>(
  value: T,
): T | string {
  if (typeof value !== "string" || value.length === 0) return value;
  return FORMULA_TRIGGER_CHARS.has(value[0]) ? `'${value}` : value;
}
function sanitizeRow<T extends Record<string, string | number | Date | null>>(
  row: T,
): T {
  const sanitized = { ...row };
...
export async function buildOutputReportWorkbook(dataset: OutputReportDataset) {
  ...
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.equipment,
    EQUIPMENT_LOG_COLUMNS,
    dataset.equipmentRows,
  );
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.safety,
    SAFETY_LOG_COLUMNS,
    dataset.safetyRows,
  );
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.measuringPoints,
    MEASURING_POINTS_LOG_COLUMNS,
    dataset.measuringPointRows,
  );
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.meterCounters,
    METER_COUNTERS_LOG_COLUMNS,
    dataset.meterCounterRows,
  );
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.shift,
    SHIFT_LOG_COLUMNS,
    dataset.shiftRows,
  );
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.kaizen,
    KAIZEN_LOG_COLUMNS,
    dataset.kaizenRows,
  );
  addDataSheet(
    workbook,
    OUTPUT_REPORT_SHEETS.master,
    MASTER_LOG_COLUMNS,
    dataset.masterRows,
  );
...
export function outputReportFilename(
  fromDate: string,
  toDate: string,
  companyName?: string,
) {
...
function addReadMeSheet(
  ...
  const rows = [
    ...
    [
      OUTPUT_REPORT_SHEETS.equipment,
      "Equipment voice/log records with maintenance helper fields.",
    ],
    [
      OUTPUT_REPORT_SHEETS.safety,
      "Safety records and available safety master snapshots.",
    ],
    [
      OUTPUT_REPORT_SHEETS.measuringPoints,
      "All measuring point readings, including normal readings.",
    ],
    [
      OUTPUT_REPORT_SHEETS.meterCounters,
      "All meter counter readings, including normal readings.",
    ],
    [OUTPUT_REPORT_SHEETS.shift, "Shift log records. Equipment is optional."],
    [OUTPUT_REPORT_SHEETS.kaizen, "Kaizen/suggestion records."],
    [
      OUTPUT_REPORT_SHEETS.master,
      "Chronological consolidation of all exported records.",
    ],
    [
      OUTPUT_REPORT_SHEETS.kpiDashboard,
      "Backend-computed KPI dashboard tables for the selected period.",
    ],
  ];
...
function addKpiDashboardSheet(
  ...
  sheet.addRow([
    "Company",
    neutralizeFormula(kpi.company.name),
    "From",
    kpi.period.fromDate,
    "To",
    kpi.period.toDate,
  ...
  addSection(
    ...
    [
      ...
      [
        "Critical + High Safety Incidents",
        kpi.summary.criticalHighSafetyIncidents,
      ],
      ["Out-of-Limit Measurements", kpi.summary.outOfLimitMeasurements],
      [
        "High-Deviation Counter Readings",
        kpi.summary.highDeviationCounterReadings,
      ],
    ...
    {
      percentCells: [
        { rowIndex: 5, columnNumber: 2 },
        { rowIndex: 11, columnNumber: 2 },
      ],
    },
  );
  ...
  addSection(
    sheet,
    "1. Mean Time To Repair (MTTR) by Equipment Category",
    ["Equipment Category", "Avg Downtime Hours", "Breakdown Count"],
    kpi.mttrByEquipmentCategory.map((row) => [
      row.equipmentCategory,
      row.averageDowntimeHours,
      row.breakdownCount,
    ]),
  );
  addSection(
    sheet,
    "2. Total Downtime Hours by Section",
    ["Section", "Total Downtime Hours", "% of Total"],
    kpi.downtimeBySection.map((row) => [
      row.section,
      row.totalDowntimeHours,
      row.percentageOfTotalDowntime,
    ]),
    { percentColumns: [3] },
  );
  addSection(
    sheet,
    "3. Top Repeat Failures",
    [
      "Equipment ID",
      "Equipment Name",
      "Function",
    ...
    kpi.repeatFailures.map((row) => [
      row.equipmentId,
      row.equipmentName,
      row.equipmentFunction,
      row.failureMode,
      row.occurrenceCount,
    ]),
  );
  addSection(
    sheet,
    "4. Mean Time Between Failures (MTBF) proxy",
    [
      "Equipment ID",
      "Equipment Name",
    ...
    kpi.mtbfProxyByEquipment.map((row) => [
      row.equipmentId,
      row.equipmentName,
      row.averageDaysBetweenFailures,
      row.failureCount,
    ]),
  );
  addSection(
    ...
    ["Maintenance Type", "Count", "% of Total"],
    kpi.maintenanceTypeDistribution.items.map((row) => [
      row.maintenanceType,
      row.count,
  ...
  addSection(
    sheet,
    "6. Production Impact of Breakdowns",
    ["Production Impact", "Count", "% of Breakdowns"],
    kpi.productionImpactBreakdowns.items.map((row) => [
      row.productionImpact,
      row.count,
      row.percentageOfBreakdowns,
    ]),
    { percentColumns: [3] },
  );
  addSection(
    sheet,
    "7. Safety Incident Count by Month",
    ["Month", "Incidents", "Critical/High"],
    kpi.safetyMonthlyTrend.map((row) => [
      row.month,
      row.incidentCount,
      row.criticalHighSeverityCount,
    ]),
  );
  addSection(
    sheet,
    "8. Safety Incident Severity Distribution",
    ["Severity", "Incidents", "% of Total"],
    kpi.safetySeverityDistribution.map((row) => [
      row.severity,
      row.incidentCount,
      row.percentageOfTotal,
    ]),
  ...
  addSection(
    sheet,
    "9. Reportable Incidents Summary",
    ["Metric", "Value"],
    [
  ...
  addSection(
    sheet,
    "10. Condition Monitoring - Out-of-Limit Frequency",
    ["Measurement", "Total", "Out of Limit", "% Out of Limit"],
    kpi.measuringPointOutOfLimit.items.map((row) => [
      row.measurementName,
      row.totalReadings,
      row.outOfLimitCount,
      row.outOfLimitPercent,
    ]),
    { percentColumns: [4] },
  );
  addSection(
    sheet,
    "11. Utility/Meter Counter - Avg Consumption Deviation %",
    [
      "Counter ID",
      "Counter",
      "Avg Deviation %",
      "High-Deviation Count",
      "Readings",
    ],
    kpi.meterCounterDeviation.items.map((row) => [
      row.counterId,
      row.counterName,
      row.averageDeviationPercent,
      row.highDeviationCount,
      row.readingCount,
    ]),
    { percentColumns: [3] },
  );
  addSection(
    sheet,
    "12. Continuous Improvement - Kaizen Status Funnel",
    ["Status", "Count"],
    kpi.kaizenStatusFunnel.items.map((row) => [row.status, row.count]),
  );
  addSection(
    sheet,
    "13. Kaizen Submissions by Category",
    ["Category", "Count", "% of Total"],
    kpi.kaizenByCategory.map((row) => [
      row.category,
      row.count,
      row.percentageOfTotal,
    ]),
...
function addSection(
  ...
  options: {
    ...
    percentCells?: Array<{ rowIndex: number; columnNumber: number }>;
  } = {},
) {
  ...
  for (const cell of options.percentCells ?? []) {
    sheet.getCell(firstDataRow + cell.rowIndex - 1, cell.columnNumber).numFmt =
      "0.00%";
  }
  sheet.addRow([]);
}
...
function columnWidth(column: string) {
  if (
    ...
    column.includes("Action") ||
    column.includes("Root Cause")
  )
    return 28;
  if (
    column.includes("Equipment Name") ||
  ...
  if (
    ...
    column.includes("Severity")
  )
    return 16;
  if (
    column.includes("Deviation") ||
    column.includes("Attachment") ||
    column.includes("Downtime")
  )
    return 18;
