...
const PLANNED_MAINTENANCE_TYPES = new Set([
  "preventive",
  "predictive",
  "inspection round",
]);
...
export function buildKpiDashboard(
  ...
  const totalDowntimeHours = round(
    sum(equipmentRows.map((row) => numberValue(row["Downtime Hours"]))),
  );
  ...
  const breakdownRows = equipmentRows.filter(
    (row) => normalize(row["Maintenance Type"]) === BREAKDOWN_MAINTENANCE_TYPE,
  );
  const plannedMaintenancePercent = percent(
    equipmentRows.filter((row) =>
      PLANNED_MAINTENANCE_TYPES.has(normalize(row["Maintenance Type"])),
    ).length,
    equipmentRows.length,
  );
  const criticalHighSafetyRows = safetyRows.filter((row) =>
    ["critical", "high"].includes(normalize(row.Severity)),
  );
  const outOfLimitRows = measuringRows.filter(
    (row) => normalize(row["Out of Limit Flag"]) === "out of limit",
  );
  const highDeviationRows = counterRows.filter(
    (row) => normalize(row["Deviation Flag"]) === "high deviation",
  );
  ...
  return {
    ...
    summary: {
      ...
      kaizenClosureRate: percent(
        kaizenRows.filter((row) =>
          CLOSED_KAIZEN_STATUSES.has(normalize(row.Status)),
        ).length,
        kaizenRows.length,
      ),
    },
    ...
    safetyMonthlyTrend: safetyMonthlyTrend(
      safetyRows,
      dataset.range.from,
      dataset.range.to,
    ),
...
function mttrByEquipmentCategory(rows: SheetRow[]) {
  const grouped = groupBy(
    rows,
    (row) => stringValue(row["Equipment Category"]) || "Unknown",
  );
  return [...grouped.entries()]
    ...
    .sort(
      (a, b) =>
        b.averageDowntimeHours - a.averageDowntimeHours ||
...
function downtimeBySection(rows: SheetRow[], totalDowntimeHours: number) {
  return [
    ...groupBy(rows, (row) => stringValue(row.Section) || "Unknown").entries(),
  ]
    .map(([section, sectionRows]) => {
      const sectionDowntime = round(
        sum(sectionRows.map((row) => numberValue(row["Downtime Hours"]))),
      );
      return {
        section,
    ...
    .sort(
      (a, b) =>
        b.totalDowntimeHours - a.totalDowntimeHours ||
  ...
  return [...grouped.values()]
    .map((groupRows) => {
      const sorted = [...groupRows].sort(
        (a, b) =>
          dateValue(a["Reported Date"]).getTime() -
          dateValue(b["Reported Date"]).getTime(),
      );
      const intervals: number[] = [];
      for (let index = 1; index < sorted.length; index += 1) {
        intervals.push(
          daysBetween(
            dateValue(sorted[index - 1]["Reported Date"]),
            dateValue(sorted[index]["Reported Date"]),
          ),
        );
      }
      const row = sorted[0];
    ...
    .sort(
      (a, b) =>
        (a.averageDaysBetweenFailures ?? 0) -
          (b.averageDaysBetweenFailures ?? 0) ||
        a.equipmentId.localeCompare(b.equipmentId),
    )
    .slice(0, 10);
}

function maintenanceTypeDistribution(
  rows: SheetRow[],
  ...
  return {
    ...
    items: countDistribution(
      rows,
      (row) => stringValue(row["Maintenance Type"]) || "Unknown",
      rows.length,
      "maintenanceType",
    ),
  };
...
function productionImpactBreakdowns(rows: SheetRow[]) {
  return {
    ...
    items: countDistribution(
      rows,
      (row) => stringValue(row["Production Impact"]) || "Unknown",
      rows.length,
      "productionImpact",
    ).map((item) => ({
...
function safetySeverityDistribution(rows: SheetRow[]) {
  return countDistribution(
    rows,
    (row) => stringValue(row.Severity) || "Unknown",
    rows.length,
    "severity",
  ).map((item) => ({
...
function measuringPointOutOfLimit(rows: SheetRow[]) {
  const items = [
    ...groupBy(
      rows,
      (row) => stringValue(row["Measurement Name"]) || "Unknown",
    ).entries(),
  ]
    .map(([measurementName, measurementRows]) => {
      const outOfLimitCount = measurementRows.filter(
        (row) => normalize(row["Out of Limit Flag"]) === "out of limit",
      ).length;
      return {
        measurementName,
    ...
    .sort(
      (a, b) =>
        (b.outOfLimitPercent ?? 0) - (a.outOfLimitPercent ?? 0) ||
        b.outOfLimitCount - a.outOfLimitCount ||
        a.measurementName.localeCompare(b.measurementName),
    );

  const totalOutOfLimitReadings = rows.filter(
    (row) => normalize(row["Out of Limit Flag"]) === "out of limit",
  ).length;
  return {
...
function meterCounterDeviation(rows: SheetRow[]) {
  const items = [
    ...groupBy(
      rows,
      (row) =>
        `${stringValue(row["Counter ID"])}|${stringValue(row["Counter Name"])}`,
    ).entries(),
  ]
    .map(([, counterRows]) => {
      ...
      return {
        ...
        highDeviationCount: counterRows.filter(
          (row) => normalize(row["Deviation Flag"]) === "high deviation",
        ).length,
    ...
    .sort(
      (a, b) =>
        b.highDeviationCount - a.highDeviationCount ||
        Math.abs(b.averageDeviationPercent ?? 0) -
          Math.abs(a.averageDeviationPercent ?? 0) ||
        a.counterName.localeCompare(b.counterName),
    );

  return {
    totalCounterReadings: rows.length,
    highDeviationCount: rows.filter(
      (row) => normalize(row["Deviation Flag"]) === "high deviation",
    ).length,
    items,
  };
...
function kaizenStatusFunnel(rows: SheetRow[]) {
  return {
    ...
    closureRate: percent(
      rows.filter((row) => CLOSED_KAIZEN_STATUSES.has(normalize(row.Status)))
        .length,
      rows.length,
    ),
    items: countDistribution(
      rows,
      (row) => stringValue(row.Status) || "Unknown",
      rows.length,
      "status",
    ).map((item) => ({
      status: item.status,
...
function kaizenByCategory(rows: SheetRow[]) {
  return countDistribution(
    rows,
    (row) => stringValue(row["Kaizen Category"]) || "Unknown",
    rows.length,
    "category",
  );
}
...
function continuousMonthBuckets(from: Date, to: Date) {
  const buckets = new Map<
    string,
    { month: string; incidentCount: number; criticalHighSeverityCount: number }
  >();
