...
export async function createSignedImageUpload(input: CreateSignedUploadBody) {
  ...
  if (!provider.createSignedUpload) {
    throw new AppError({
      message:
        "Signed uploads are only available for the active storage provider when supported.",
...
async function resolveAssetOwner(key: string): Promise<AssetOwner | null> {
  ...
  const [log] = await db
    ...
    .where(eq(operationalLogs.voiceRecordingKey, key))
    .limit(1);
  if (log) return { scope: "company", companyId: log.companyId };

  const [manual] = await db
    .select({ companyId: equipmentManuals.companyId })
    .from(equipmentManuals)
    .where(eq(equipmentManuals.fileKey, key))
    .limit(1);
  if (manual) return { scope: "company", companyId: manual.companyId };

  const [admin] = await db
    .select({ companyId: admins.companyId })
    .from(admins)
    .where(eq(admins.avatarKey, key))
    .limit(1);
  ...
  const [company] = await db
    ...
    .from(companies)
    .where(eq(companies.logoKey, key))
    .limit(1);
  ...
  const [moduleRow] = await db
    ...
    .from(modules)
    .where(eq(modules.mediaKey, key))
    .limit(1);
  if (moduleRow) return { scope: "platform" };

  const [generalSettings] = await db
    .select({ id: platformGeneralSettings.id })
    .from(platformGeneralSettings)
    .where(eq(platformGeneralSettings.logoKey, key))
    .limit(1);
...
async function authorizeAssetAccess(
  ...
  const forbidden =
    owner.scope === "platform"
      ? !isMaster
      : !isMaster && owner.companyId !== caller.companyId;
...
export async function getSignedMediaUrl(
  key: string,
  resourceType: "image" | "video" | "raw" | undefined,
  caller: { companyId?: string; role: string },
) {
